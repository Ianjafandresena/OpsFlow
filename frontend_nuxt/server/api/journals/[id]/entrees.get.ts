...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const entrees: any[] = await prisma.entreeJournal.findMany({
      ...
      include: {
        tache: {
          select: { id: true, titre: true, lien_livrable: true, aVerifier: true, motifModification: true, urgent: true, statutTache: { select: { libelle: true } } }
        },
          ...
          for (const task of tasksToAdd) {
            ...
            try {
              const newEntry = await prisma.entreeJournal.create({
                ...
                include: {
                  tache: { select: { id: true, titre: true, lien_livrable: true, aVerifier: true, motifModification: true, urgent: true, statutTache: { select: { libelle: true } } } },
                  editeur: { select: { id: true, nom: true, prenom: true } },
