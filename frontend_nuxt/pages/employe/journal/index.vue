      ...
      <div v-if="selectedJournal" class="card journal-card">
        ...
        <div class="journal-card-header">
          <div>
            ...
            <div class="viewer-members">
              <span
                v-for="(emp, idx) in journalMembres"
                :key="emp.id"
                class="member-chip"
                :class="memberChipClass(emp)"
            ...
            <tbody>
              <tr v-for="slot in timeSlots" :key="slot" :class="{ 'current-time-row': isCurrentSlot(slot) }">
                ...
                <td
                  ...
                  :key="emp.id"
                  class="entry-cell"
                  :class="emp.id === myEmployeId ? 'entry-cell-mine' : 'entry-cell-partner'"
                >
                  <div class="cell-wrapper" v-if="localGrid[emp.id] && localGrid[emp.id][slot] !== undefined">
                    <!-- Auto from task -->
                    <div
                      v-if="getEntryRaw(emp.id, slot)?.tacheId && !isEditing(emp.id, slot)"
                    ...
                    >
                      <div style="display:flex; justify-content:space-between; width:100%; align-items:flex-start;">
                        ...
                        <div style="display:flex; gap:0.15rem;">
                          ...
                          <button
                            v-if="getEntryRaw(emp.id, slot)?.admin_commentaire"
                            class="icon-btn notif-btn"
                            @click="openEntryModal(emp.id, slot)"
                            title="Commentaire de l'admin"
                          >
                            <MessageSquareIcon :size="12" />
                            <span class="notif-badge">1</span>
                          </button>
                          <!-- Evaluation badge -->
                          <span v-if="getEntryRaw(emp.id, slot)?.evaluation_type && getEntryRaw(emp.id, slot)?.evaluation_type !== 'NEUTRE'"
                            :class="evalTagClass(getEntryRaw(emp.id, slot)?.evaluation_type)" class="eval-tag-emp">
                    ...
                    <div v-else class="edit-stack">
                      <div style="position:relative;">
                        <textarea
                          ...
                          :class="{ 'entry-textarea-filled': localGrid[emp.id][slot].contenu && localGrid[emp.id][slot].contenu.trim() }"
                        ...
                        <div class="cell-actions">
                          ...
                          <button type="button" @click="openEntryModal(emp.id, slot)" class="icon-btn" title="Détails" style="opacity: 0.6;">
                            <EyeIcon :size="14" />
                          </button>
                        </div>
                      </div>
                      ...
                      <template v-if="localGrid[emp.id][slot].lien">
                        <a v-for="(lnk, idx) in localGrid[emp.id][slot].lien.split(/[\s\n]+/).filter(l => l.trim() !== '')" :key="idx"
                           :href="lnk" target="_blank" class="entry-link-chip" @click.stop>
                          <LinkIcon :size="10" /> Lien {{ idx + 1 }}
                        </a>
                      </template>
                      <!-- Evaluation badge -->
                      <span v-if="getEntryRaw(emp.id, slot)?.evaluation_type && getEntryRaw(emp.id, slot)?.evaluation_type !== 'NEUTRE'"
                        :class="evalTagClass(getEntryRaw(emp.id, slot)?.evaluation_type)" class="eval-tag-emp eval-tag-emp-inline">
        ...
        <div v-else-if="viewMode === 'semaine'" class="journal-grid-wrapper">
          <div style="display:flex; gap: 0.5rem; padding: 0.5rem 1rem; background: var(--bg-surface-hover); border-bottom: 1px solid var(--border-light);">
            ...
            >{{ emp.id === myEmployeId ? 'Moi' : emp.prenom }}</button>
          </div>
          <table class="journal-table">
            <thead>
              <tr>
          ...
          <div class="modal-body" style="display:flex; flex-direction:column; gap:1rem;">
            <div class="form-group">
              ...
              <textarea
                ...
                class="form-input" rows="3" placeholder="Ex: Rédaction du rapport..." style="resize:vertical;"
              ></textarea>
            </div>
            ...
            <div class="form-group">
              ...
              <textarea
                ...
                class="form-input" rows="2" placeholder="Ex: En attente de validation par le client" style="resize:vertical;"
              ></textarea>
            </div>
            ...
            <div class="form-group">
              ...
              <textarea
                ...
                class="form-input" rows="2" placeholder="https://lien1.com&#10;https://lien2.com" style="resize:vertical; font-size: 0.8125rem;"
              ></textarea>
            </div>

            <!-- Admin Comment (read-only for employee) -->
            <div v-if="getEntryRaw(activeEmployeId, activeSlot, activeDate)?.admin_commentaire" class="admin-msg-box">
              <div class="admin-msg-label"><MessageSquareIcon :size="12" /> Commentaire de l'Administrateur</div>
              <div class="admin-msg-content">{{ getEntryRaw(activeEmployeId, activeSlot, activeDate).admin_commentaire }}</div>
            </div>

            <!-- Evaluation badge (read-only) -->
...
const journalMembres = computed(() => {
  ...
  return [
    selectedJournal.value.employe1,
    selectedJournal.value.employe2,
    selectedJournal.value.employe3,
    selectedJournal.value.employe4
  ].filter(Boolean)
})
...
const loadJournals = async () => {
  ...
  try {
    ...
    myJournals.value = allJournals.filter(j =>
      ...
      j.employe3Id === myEmployeId.value ||
      j.employe4Id === myEmployeId.value
    )
  ...
  if (viewMode.value === 'journalier') {
    journalMembres.value.forEach(emp => {
      ...
      timeSlots.forEach(slot => {
        ...
        grid[emp.id][slot] = { contenu: e?.contenu || '', commentaire: e?.commentaire || '', lien: e?.lien || '' }
      })
      ...
      journalMembres.value.forEach(emp => {
        timeSlots.forEach(slot => {
          ...
          if (cell && (cell.contenu.trim() || cell.commentaire.trim() || cell.lien.trim())) {
            entrees.push({ employeId: emp.id, heure: slot, contenu: cell.contenu.trim(), commentaire: cell.commentaire.trim(), lien: cell.lien.trim() })
          }
        })
...
.journal-tabs { display: flex; gap: 0.25rem; margin-bottom: 1rem; border-bottom: 1px solid var(--border-light); }
...
/* Notification badge for admin comments */
...
.entry-link-chip { display: inline-flex; align-items: center; gap: 0.2rem; color: var(--accent-primary); font-size: 0.7rem; text-decoration: none; padding: 0.1rem 0.35rem; border-radius: 4px; background: var(--accent-primary)10; border: 1px solid var(--accent-primary)20; margin-top: 0.15rem; }
...
/* Admin message box */
.admin-msg-box { background: #ef444408; border: 1px solid #ef444430; border-radius: 10px; padding: 0.75rem 1rem; }
.admin-msg-label { font-size: 0.7rem; font-weight: 700; color: #dc2626; text-transform: uppercase; letter-spacing: 0.04em; display: flex; align-items: center; gap: 0.35rem; margin-bottom: 0.4rem; }
.admin-msg-content { font-size: 0.8125rem; color: #b91c1c; line-height: 1.5; }
