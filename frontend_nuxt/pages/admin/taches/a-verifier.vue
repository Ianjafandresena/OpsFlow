    ...
    <div v-else style="display:flex;flex-direction:column;gap:1.25rem;">
            ...
            <tr v-for="t in taches" :key="t.id" class="verif-row">
              ...
              <td style="text-align:right;">
                <div style="display:flex;gap:0.5rem;justify-content:flex-end;">
                  ...
                  <button class="btn btn-sm btn-warning" @click="openModal('tache', t)"><EditIcon :size="13" /> À modifier</button>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div v-if="entreesManuelles.length > 0" class="card" style="padding:0;overflow:hidden;">
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="e in entreesManuelles" :key="e.id" class="verif-row">
              <td>
                <div class="emp-cell">
              ...
              <td style="text-align:right;">
                <div style="display:flex;gap:0.5rem;justify-content:flex-end;">
                  ...
                  <button class="btn btn-sm btn-warning" @click="openModal('entree', e)"><EditIcon :size="13" /> À modifier</button>
                </div>
              </td>
            </tr>
          </tbody>
        ...
        <div class="modal-box">
          ...
          <div class="modal-body">
            <p style="font-size:0.875rem;color:var(--text-secondary);margin-bottom:0.75rem;">
              <span v-if="modalType==='tache'">Tâche : <strong>{{ selectedItem?.titre }}</strong></span>
              <span v-else>Note : <strong>{{ selectedItem?.contenu?.substring(0,60) }}{{ selectedItem?.contenu?.length > 60 ? '...' : '' }}</strong></span>
              <br>Employé : <strong>{{ selectedItem?.employe?.prenom }} {{ selectedItem?.employe?.nom }}</strong>
            </p>
            <div class="form-group">
              <label class="form-label">Motif <span style="color:#ef4444;">*</span></label>
              <textarea v-model="motif" class="form-input" rows="4" placeholder="Décrivez précisément ce qui doit être corrigé..." style="resize:vertical;"></textarea>
            </div>
          </div>
...
const modalType = ref('tache') // 'tache' | 'entree'
...
const envoyerModification = async () => {
  if (!motif.value.trim() || !selectedItem.value) return
  submitting.value = true
  try {
    if (modalType.value === 'tache') {
      await $fetch(`/api/taches/${selectedItem.value.id}/modifier-demande`, { method: 'POST', body: { motif: motif.value.trim() } })
      taches.value = taches.value.filter(t => t.id !== selectedItem.value.id)
    } else {
      await $fetch(`/api/entrees/${selectedItem.value.id}/modifier-demande`, { method: 'POST', body: { motif: motif.value.trim() } })
