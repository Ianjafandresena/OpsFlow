<template>
  <div class="animate-fade-in">
    <div class="page-header">
      <div>
        <h1 class="page-title">Tâches Designers</h1>
        <p class="page-subtitle" style="margin-bottom:0;">Suivi des créations graphiques (affiches, caroussels IG).</p>
      </div>
      <button class="btn btn-primary" @click="openCreate">
        <PlusIcon :size="15" /> Nouveau Livrable
      </button>
    </div>
    ...
    <div class="card" style="margin-bottom:1.25rem; display:flex; flex-direction:row; gap:0.75rem; align-items:center; flex-wrap:wrap; padding:0.875rem 1.25rem;">
      <div class="search-wrapper" style="flex:1; min-width:160px;">
        ...
        <input type="text" v-model="filters.search" placeholder="Rechercher un visuel..." class="form-input" />
      </div>
      ...
      <select v-model="filters.employeId" class="form-input" style="width:auto; min-width:150px;">
        ...
        <option v-for="emp in designerEquipe" :key="emp.id" :value="emp.id">{{ emp.nom }} {{ emp.prenom }}</option>
      </select>
      ...
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr v-if="filteredTasks.length===0">
            <td colspan="8" style="text-align:center; padding:2rem; color:var(--text-secondary);">Aucun livrable trouvé.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <!-- Modal CRUD -->
    <Modal :isOpen="modal" :title="editing ? 'Modifier le Livrable' : 'Nouveau Livrable'" @close="modal=false">
      <form @submit.prevent="save" style="display:flex; flex-direction:column; gap:1rem;">
        ...
        <div class="form-group">
          <label class="form-label">Attribuer à (Designer)</label>
        ...
        <div class="form-group">
          <label class="form-label">Brief & Instructions</label>
          <textarea v-model="form.description" class="form-input" rows="3" placeholder="Dimensions, couleurs, ressources existantes..."></textarea>
        </div>
...
const { data: taches, refresh: refreshTaches } = await useFetch('/api/taches', { query: { typeTache: 'DESIGNER' } })
...
// Filtre l'équipe pour les designers
...
const defaultForm = () => {
  ...
  return { 
    id:null, titre:'', employeId:'', editionId:'', statutTacheId: defaultStatut, date_limite:'',
    type_visuel:'Affiche', quantite: 1, description:'' 
  }
}
...
const remove = (id) => { 
  requireConfirm('Supprimer ce livrable ?', 'Êtes-vous sûr de vouloir supprimer ce livrable ?', async () => {
