...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "animate-fade-in" }, _attrs))}><div class="page-header"><div><h1 class="page-title">Affectations</h1><p class="page-subtitle" style="${ssrRenderStyle({ "margin-bottom": "0" })}">Assignez les collaborateurs aux pages d&#39;événements spécifiques.</p></div><button class="btn btn-primary">`);
      ...
      _push(`<div><strong>Principe :</strong> Chaque affectation lie un collaborateur à un événement dans une ville. Un CM affecté à « JOF - Évreux » ne verra que cette page dans son espace personnel. Un collaborateur peut être affecté à plusieurs pages. </div></div><div class="card" style="${ssrRenderStyle({ "padding": "0", "overflow": "hidden" })}"><table class="data-table"><thead><tr><th>Collaborateur</th><th>Poste</th><th>Événement</th><th>Ville</th><th style="${ssrRenderStyle({ "text-align": "right" })}">Actions</th></tr></thead><tbody><!--[-->`);
      ...
      if (affectationsList.value.length === 0) {
        _push(`<tr><td colspan="5" style="${ssrRenderStyle({ "text-align": "center", "padding": "2rem", "color": "var(--text-secondary)" })}">Aucune affectation trouvée.</td></tr>`);
      } else {
        _push(`<!---->`);
      }
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            ssrRenderList(unref(employes), (emp) => {
              _push2(`<option${ssrRenderAttr("value", emp.id)}${ssrIncludeBooleanAttr(Array.isArray(form.value.employeId) ? ssrLooseContain(form.value.employeId, emp.id) : ssrLooseEqual(form.value.employeId, emp.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(emp.nom)} ${ssrInterpolate(emp.prenom)} (${ssrInterpolate(emp.poste?.titre_poste)})</option>`);
            });
            _push2(`<!--]--></select></div><div class="form-group"${_scopeId}><label class="form-label"${_scopeId}>Édition (Page)</label><select required class="form-input"${_scopeId}><option value="" disabled${ssrIncludeBooleanAttr(Array.isArray(form.value.editionId) ? ssrLooseContain(form.value.editionId, "") : ssrLooseEqual(form.value.editionId, "")) ? " selected" : ""}${_scopeId}>Choisir une édition</option><!--[-->`);
            ...
            return [
              createVNode("form", {
                  ...
                  withDirectives(createVNode("select", {
                    ...
                    (openBlock(true), createBlock(Fragment, null, renderList(unref(employes), (emp) => {
                      return openBlock(), createBlock("option", {
                        key: emp.id,
                        value: emp.id
                      }, toDisplayString(emp.nom) + " " + toDisplayString(emp.prenom) + " (" + toDisplayString(emp.poste?.titre_poste) + ")", 9, ["value"]);
                    }), 128))
                ...
                createVNode("div", { class: "form-group" }, [
                  createVNode("label", { class: "form-label" }, "Édition (Page)"),
                  withDirectives(createVNode("select", {
...
//# sourceMappingURL=index-DL6ss0iS.js.map
