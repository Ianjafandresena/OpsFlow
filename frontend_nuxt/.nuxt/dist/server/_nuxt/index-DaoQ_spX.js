...
import { _ as __nuxt_component_0$1 } from "./Modal-D-MuMCI8.js";
...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<button class="btn btn-secondary" style="${ssrRenderStyle({ "width": "100%", "justify-content": "center", "padding": "0.75rem 1.5rem", "font-size": "0.9375rem" })}"> Espace Employé (Connexion) </button></div><p style="${ssrRenderStyle({ "color": "var(--text-muted)", "font-size": "0.75rem", "margin-top": "2rem" })}">v1.0 — Architecture Modulaire Nuxt 3 + PostgreSQL</p></div>`);
      _push(ssrRenderComponent(_component_Modal, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div style="${ssrRenderStyle({ "display": "flex", "flex-direction": "column", "gap": "1rem" })}"${_scopeId}><p style="${ssrRenderStyle({ "color": "var(--text-secondary)", "font-size": "0.875rem" })}"${_scopeId}>Sélectionnez un profil d&#39;employé pour tester l&#39;affichage dynamique :</p><select class="form-input" style="${ssrRenderStyle({ "width": "100%" })}"${_scopeId}><option value="" disabled${ssrIncludeBooleanAttr(Array.isArray(selectedEmployeId.value) ? ssrLooseContain(selectedEmployeId.value, "") : ssrLooseEqual(selectedEmployeId.value, "")) ? " selected" : ""}${_scopeId}>Choisir un profil...</option><!--[-->`);
            ssrRenderList(unref(employes), (emp) => {
              _push2(`<option${ssrRenderAttr("value", emp.id)}${ssrIncludeBooleanAttr(Array.isArray(selectedEmployeId.value) ? ssrLooseContain(selectedEmployeId.value, emp.id) : ssrLooseEqual(selectedEmployeId.value, emp.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(emp.nom)} ${ssrInterpolate(emp.prenom)} (${ssrInterpolate(emp.poste?.titre_poste || "Inconnu")}) </option>`);
            });
            ...
            return [
              createVNode("div", { style: { "display": "flex", "flex-direction": "column", "gap": "1rem" } }, [
                createVNode("p", { style: { "color": "var(--text-secondary)", "font-size": "0.875rem" } }, "Sélectionnez un profil d'employé pour tester l'affichage dynamique :"),
                withDirectives(createVNode("select", {
                  ...
                  (openBlock(true), createBlock(Fragment, null, renderList(unref(employes), (emp) => {
                    return openBlock(), createBlock("option", {
                      key: emp.id,
                      value: emp.id
                    }, toDisplayString(emp.nom) + " " + toDisplayString(emp.prenom) + " (" + toDisplayString(emp.poste?.titre_poste || "Inconnu") + ") ", 9, ["value"]);
                  }), 128))
...
//# sourceMappingURL=index-DaoQ_spX.js.map
